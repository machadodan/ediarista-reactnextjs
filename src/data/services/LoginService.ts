...
import { CredenciaisInterface } from "../@types/FormInterface";
import { UserInterface } from "../@types/UseInterface";
...
export const LoginService = {
  async login(credentials: CredenciaisInterface): Promise<boolean> {
    ...
    try {
      const { data } = await ApiService.post<{access: string; refresh: string}>(
      "/auth/token/", 
      credentials
      );
  ...
  async logout(): Promise<void> {
    refresh: ApiService.post("/auth/logout/", {
